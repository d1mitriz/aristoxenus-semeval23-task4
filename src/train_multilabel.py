...
from model.dataset import ValueEvalDataLoader, UnifiedTaptValueEvalDataLoader
from model.trainer import Trainer
from model.model import (TransformerClassifier,
                         TaptTransformerClassifier,
...
# from src.utils.configs import DotDict
...
def train_ova(__configs: dict, __label_index: int) -> None:
    ...
    set_seed(__configs['seed'])  # , pooling_operation=configs["pooling_operation"]

    tokenizer = AutoTokenizer.from_pretrained(__configs['pretrained_model'])
    pr_model = AutoModel.from_pretrained(__configs['pretrained_model'], output_hidden_states=True)

    print(__configs['pretrained_model'])

    if __configs['tapt']:
        if __configs["pooling_operation"] == 'mean_max':
            model = TaptTransformerForMeanMaxPoolingClassifier(
                model=pr_model,
        ...
        elif __configs["pooling_operation"] == 'attention':
            model = TaptTransformerForAttentionPoolingClassifier(
                model=pr_model,
                dropout=__configs['dropout'],
                n_classes=__configs['n_classes'],
                pooling=__configs['pooling'],
...
if __name__ == '__main__':
    ...
    for label_index in range(labels):
        ...
        configs.update({
            'model_folder': f'multilabel_train/bbu/{label_index}',
            # 'model_folder': f'multilabel_train/{configs["pooling_operation"]}/{label_index}',
            'model_name': f'{configs["pretrained_model"].replace("/", "--")}_label_{label_index}_',
            'experiment_name': f'Multilabel Train with BBU {configs["pooling_operation"]}',
            'run_name': f'{configs["pretrained_model"].replace("/", "--")}_label_{label_index}'
        })
